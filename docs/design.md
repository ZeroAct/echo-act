...
| ID | Feature | Specification and verification criteria |
| --- | --- | --- |
| F-04 | Model selection | One of Supertonic 3, Qwen3-TTS 0.6B CustomVoice, and Qwen3-TTS 1.7B CustomVoice is selected. The default model is Supertonic 3. The choices are three checkpoints across two model families. |
...
| F-06 | Gender and voice selection | A female or male voice is selected, along with a voice available for that model and gender. Supertonic provides 5 female and 5 male voices; Qwen provides 4 female and 5 male voices. A voice's own native language may differ from the narration language, and not every model offers a native voice for every language and gender combination. Where a voice is used outside its native language the app says so before generation rather than presenting the result as native, and quality in that case is governed by N-08. |
...
| F-08 | Speaking style selection | Natural, Calm, Bright, or Narration is selected. Qwen 1.7B honors expressive instructions; the other two models apply it as a preset centered on tempo and inter-sentence pauses. |
...
| F-16 | Save audio | After full generation completes, the audio is saved as a single WAV file, in the format defined by F-82, at a location chosen by the user. The GUI does not provide a way to export only part of an in-progress or canceled job to a file. Retrieval of ready segments through external integrations follows F-55. |
...
| F-50 | Ownership and permissions | By default a client can query and cancel only its own jobs and results. Library access requires separate authorization. The GUI owner can review and cancel all jobs. External integrations cannot raise the global resource budget or halt other jobs. |
...
| F-54 | Generation request and status query | A job is created from text or a TXT/Markdown upload to be validated, voice settings, and a retention flag. Required values, ranges, and mutual contradictions are validated, and unknown models or options are not arbitrarily substituted. Upload is an alternative to the generation request body and cannot be specified together with inline text. |
...
| POST /api/v1/jobs | Create from text JSON or text file upload | New acceptance 202 + job ID; duplicate re-request 200 + existing ID |
...
| N-05 | Laptop suitability | The app must be able to run on CPU without a discrete GPU and does not automatically occupy the GPU. Because required memory and speed differ by model, large models may not run under low limits. |
...
| Job | ID, request path and owner, state, source-text snapshot, voice settings, applied budget, start and end times, error code, retention flag | Per one-off and retention policy. Must not be exposed to another owner. |
...
| A-15 | N-08, N-10, F-04 | On per-OS Windows and macOS install environments, verify launch, Korean and English generation, playback, database, REST, and MCP. Record each model's minimum execution budget and unsupported combinations. |
...
- The memory budget under which each model can run and its limitations are included in the release materials, and models that cannot run within the limits are clearly identified.
...
Speech recognition is a planned second capability for EchoAct. This release is speech generation only, and Sections 2.7 and 7 state the shipped scope accurately: no requirement, endpoint, tool, setting, or user-visible string in the MVP may hint that recognition is coming.

Adding it later is cheap in the engine and expensive in the interfaces, because once a client depends on a path or a tool name, changing it is a breaking change. The items worth settling while nothing external is committed:

| Item | Decision to settle before the interface contract is published |
| --- | --- |
| Job namespace | `POST /api/v1/jobs` currently means "create a speech generation job". Decide whether the resource is namespaced, for example `/api/v1/tts/jobs`, or carries a discriminator. Recommendation: add a job kind field to the request and response now, accepting only one value, and leave the paths as they are. A field with one legal value reveals nothing and costs nothing; retrofitting a second job kind onto an unnamespaced path later does not work. |
| MCP tool names | The tool names are already qualified with `speech`, which reads as generation. `create_speech` and `get_speech_job` would sit awkwardly beside recognition tools, and renaming published tools breaks every client configuration that referenced them. Decide the naming convention once, now. |
...
No recognition work, model selection, or licensing review is in the MVP.
...
Product judgments deliberately left open. Each is a live tension in the current requirements rather than an oversight. The model-lineup entry is no longer speculative; A.5 has the measurements behind it.

| Ref | Tension | Recommendation |
| --- | --- | --- |
| F-03, F-16, F-47, Section 7 | The 50,000-character limit is roughly three hours of Korean narration. It holds the only generation slot for that whole time, F-16 forbids exporting part of an in-progress GUI job, and Section 7 excludes resuming a canceled one, so any interruption loses everything. | Relax one of the four. Allowing export of the completed portion is the smallest and least visible change. |
| F-19 | The model is released on cancellation, so the common cancel, adjust, regenerate loop pays a full reload every time and F-17's warm start does not help where it matters most. | Supertonic loads warm in 0.8 seconds, so for the default model this costs little and the requirement can stand. It matters only for a model heavy enough to be worth keeping resident, which is the same set of models A.5 says may not ship. Revisit only if one does. |
| F-04, F-21, F-23, N-05, Section 8.1 | The lineup assumes three interchangeable models. Measured, they are not: Supertonic runs inside the default budget with room to spare, while the Qwen checkpoints exceed it on memory before speed is even considered. The 1.7B needs roughly 8 GB, which is impossible on the 8 GiB minimum in Section 8.1 and sits exactly at F-21's ceiling of half of total RAM on a 16 GiB machine. The 0.6B needs roughly 3 GB, above F-23's 2 GiB floor and above F-21's default on an 8 GiB machine. | Ship Supertonic. Re-tier the Qwen checkpoints as opt-in models with a stated minimum budget and a stated minimum machine, or drop them. Do not present three models as equivalent choices in one list when one of them cannot start on the minimum supported configuration. |
| F-04, F-06, N-08 | Only one of Qwen's nine speakers is natively Korean, and she is female. A Korean male voice exists only by using a non-Korean speaker cross-lingually, which the model supports but does not recommend for quality. | If the Qwen checkpoints ship, F-06's amended wording covers it, but the Korean male combination needs an explicit quality review in the Section 8.2 sample set before release rather than being discovered by users. |
| F-27, N-08 | The engine vocalises emoji rather than skipping them, so a document containing them is read aloud with whatever the model makes of them, which A.5 measured at 1.32 seconds for three characters. F-27 forbids dropping characters, but reading them aloud is unlikely to be what a listener wants. | Decide between speaking them, skipping them while keeping their source range attached to a neighbouring segment as F-27 already provides for, or making it a setting. Skipping is the likely answer and costs nothing to implement, but it is a behaviour change the requirement should state rather than leave to the engine. |
| F-47, F-50 | External requests take the single slot and the owner can reclaim it only by cancelling manually, so a client running at the 10-per-minute rate limit can keep the desktop user locked out. | Either give GUI requests a confirmed preemption path, or state plainly that they have no priority. |

### A.4 Verification Backlog

Resolved items move to A.5. What remains open:

- Whether a first-party CPU inference path for the Qwen checkpoints will exist on Windows. Today there is none: the official Python package requires Apple's MLX, the fastest third-party engine builds only for WSL2, and the ONNX conversions are unaffiliated community uploads with negligible download counts, one of which is 12.87 GiB on disk. N-10 forbids asking the user to install anything extra, and F-84 requires pinned provenance, so shipping these models means either an official export appears or the project owns a conversion and its validation. Cost this before committing to F-04's lineup.
...
- Legal review of the responsible-AI license clause reserving the licensor's right to restrict use of the model remotely and asking distributors to make reasonable efforts to stay current. Nothing obliges the product to implement remote control, and N-01 and F-75 forbid acting without the user, but the obligation should be read against a product that is deliberately offline and update-on-consent.
