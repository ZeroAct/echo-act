...
| F-06 | Gender and voice selection | A female or male voice is selected, along with a voice available for that model and gender. Supertonic provides 5 female and 5 male voices; Qwen provides 4 female and 5 male voices. |
...
| F-80 | Help and policy review | Supported formats and per-model constraints, storage locations and retention policy, the meaning of resource limits, integration settings, personal data handling, licenses, and version information can be reviewed inside the app. Basic help is readable offline. |
...
| N-11 | Usage rights | The license and usage restrictions applicable to each model are honored. The fact that a model can be used locally does not imply unlimited use for any purpose or the right to redistribute. |
...
| F-19 | The model is released on cancellation, so the common cancel, adjust, regenerate loop pays a full reload every time and F-17's warm start does not help where it matters most. | Release on error and on memory protection; stay warm on user cancellation. |
| F-04, N-05, N-07, Section 8.2 | At the Section 8.2 baseline of 20 percent CPU and a 4 GiB budget, a 1.7B model is unlikely to synthesize faster than real time on CPU. That does not make F-12 slower, it makes it stall at every segment boundary. | Benchmark all three checkpoints before any UI work and publish the per-model minimum budget table Section 8.3 already requires. Drop or re-tier the largest model if it cannot stream. |
...
External facts these requirements depend on that have not been confirmed yet.
