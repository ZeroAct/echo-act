...
| ID | Feature | Specification and verification criteria |
| --- | --- | --- |
| F-46 | Integration activation | The local REST service is on by default. It binds the loopback address only, always requires authentication, and can be turned off by the owner. MCP is on by default and can be turned off by the owner as well. Because the MCP server is a REST client per F-58, turning REST off also makes MCP unavailable, and that consequence is stated at the point of the change. The activation state, bind address and port, connection method, and granted permissions can be reviewed. |
| F-47 | Shared features and budget | All entry paths use the same model list, input validation, job states, and resource budget. Across the GUI and all clients combined, concurrent generation is limited to one. A second request is refused with a busy error carrying a retry-after hint, and no automatic queue is provided. Contention is the expected case rather than an edge case, because the desktop user and every client share the one slot, so a client must honour the hint rather than retry immediately and must be able to give up. |
...
| F-49 | Re-requests and cancellation | Duplicate generation is prevented using a retry identifier supplied by the requester. The identifier is required on every generation request rather than optional, and a request without one is refused: with a single generation slot, a client that retries after a dropped or slow response would otherwise queue a second rendering of the same text behind the first, and automated clients retry as a matter of course. The same identifier with the same content returns the same job, while different content is refused as a conflict. Cancellation is provided as explicit job cancellation, and repeated cancellation adds no further side effects. |
...
| F-51 | Playback of external requests | External requests perform speech generation only and never play by themselves. Audio reaches the host speakers only when the client asks for it under F-89 and the owner has left that setting on; otherwise the user selects and plays that job in the GUI. In either case external requesters do not overwrite UI input or take over the user's playback. |
| F-89 | External playback request | An external client may ask for a job it created to be played on the host output device. The owner turns this on and off and it is on by default; while it is off a play request is refused as a permanent error and nothing is heard. Playback begins at the first ready segment and follows generation as F-12 describes, so a short request is spoken without waiting for the whole job. It never takes over the person at the keyboard: the input surface is left exactly as it is, its highlight is reported unavailable for the duration rather than re-anchored to the external job's text, and a request arriving while the owner's own playback is in progress is refused as busy with a retry-after hint per F-57. The owner's own play and stop always win, and stopping external playback is reachable from the main screen per F-69. Volume, mute, and output device follow F-67. Where there is no output device, generation still succeeds and only playback is reported as failed. Asked for together with generation, a refusal to play is reported as part of the job rather than as the request's failure: the job exists, its audio is retrievable, and an error there would invite a retry that F-49 answers with the same job. |
...
| ID | Feature | Specification and verification criteria |
| --- | --- | --- |
| F-53 | Service and capability query | Uptime state, API version, supported models with their native sample rate and minimum resource budget, voices, languages, speaking styles, input formats, and the actual resource policy can be queried. Each voice carries a stable identifier, a display name, and a short description of its character and intended use, because a bare identifier gives a caller that is not a person no basis for choosing one. Descriptions come from the manifest in F-84 and are shown in the GUI as well. An undownloaded model is distinguished from a server fault. |
| F-54 | Generation request and status query | A job is created from text or a TXT/Markdown upload to be validated, voice settings, a retention flag, and the duplicate-prevention key required by F-49. The request may ask to wait for completion within the bound in F-88, which changes only when the response is sent and never what the job does. Every job request and every job representation carries an explicit job kind, and this release accepts exactly one value; an unrecognised kind is refused rather than defaulted, so that a client always states what it is asking for. Required values, ranges, and mutual contradictions are validated, and unknown models or options are not arbitrarily substituted. Voice settings may be omitted in whole or in part, in which case the model manifest's documented default voice, style, and tempo apply and the job representation states the values actually used; those defaults come from the manifest and never from whatever the owner last selected in the GUI, so an automated caller's output does not depend on it. A request may also ask for the result to be played on the host output device, per F-89. Upload is an alternative to the generation request body and cannot be specified together with inline text. |
...
| F-88 | Bounded wait and pre-flight estimate | Two facilities keep a caller from having to poll blindly. A generation request may ask to wait for the job to finish, for at most a bound the owner configures and the service caps; the service never waits longer than asked, never waits on model preparation or download, and returns the job ID unchanged when the bound passes, so waiting is an optimisation and never a different job lifecycle. Separately, a caller may submit text and settings for an estimate, receiving validation results, the segment count, the estimated audio duration and synthesis time, and whether the single generation slot is currently free, without creating a job, loading a model, or synthesising anything. Estimates are derived from the model's recorded throughput and are explicitly approximate. |
...
| POST /api/v1/estimate | Validate text and settings and estimate segments, duration, and synthesis time without creating a job | 200 |
| POST /api/v1/jobs | Create from text JSON or text file upload, with an explicit job kind and duplicate-prevention key; voice settings optional; may request a bounded wait and may ask for playback per F-89 | New acceptance 202 + job ID; completed inside a requested wait 200 + terminal state; duplicate re-request 200 + existing ID. A playback request made here is reported in the job representation, refusal included, because the job was created either way |
...
| POST /api/v1/jobs/{id}/play | Ask for the job's audio to be played on the host output device, per F-89 | Accepted 202; playback turned off by the owner 403; the owner's own playback in progress 409 + retry-after |
...
| --- | --- |
| list_models | Query models, voices, languages, speaking styles, and input limits, each voice with a description |
| estimate_speech | Validate text and settings and estimate segments, duration, and synthesis time without creating a job |
| create_speech | Accept a job from text and settings, with a required duplicate-prevention key and an optional bounded wait. File and URL inputs are not accepted. |
...
| speak | Generate and speak in one call: text with the voice settings optional, a short bounded wait, and playback requested per F-89. The tool a caller reaches for when a person said "speak" or "read this aloud" |
| play_speech | Ask for one's own job to be played on the host output device, per F-89 |
...
| F-70 | Completion and error notifications | Generation completion, download failure, resource shortage, and backup failure are notified inside the app. A completion notice for a job an integration created names the client that asked for it and offers to open that job, and where the result is one-off it says when it expires, so that work produced while the user was not watching can still be found and saved before it is cleaned up. OS notifications are provided only if the user turns them on and do not contain body text or audio. Clicking a notification does not auto-play audio. |
...
| N-22 | Integration responsiveness | Request acceptance, queries, and cancellation must respond independently of model loading and generation. In the reference environment of Section 8, request acceptance, query, cancellation, and estimate responses must be within 1 second at p95, and resource release for a generation job after cancellation is accepted must be within 5 seconds. A request that opts into the bounded wait in F-88 is excluded from that figure and measured against the bound it asked for, since there the delay is the caller's instruction rather than the service's latency. Model download and generation times and large backup times are measured separately. |
...
| External connections | REST on by default, loopback only, authentication always required. MCP on by default, stdio only, and implemented as a REST client, so it is unavailable while REST is off. No independent background service, and nothing listens once the app has exited. |
...
| Bounded wait | A generation request may ask to wait for completion for up to 10 seconds, with an owner-configurable ceiling of 60 seconds. The service never waits longer than the caller asked, never waits while a model is being prepared or downloaded, and returns the job ID when the bound passes. Waiting does not extend the request-rate allowance or hold a second slot. |
...
| External playback | Playing an external request on the host output device is allowed by default and can be turned off by the owner. It is refused while the owner's own playback is in progress, and it never changes the input text or its highlight. |
...
- GUI generation auto-plays when the first segment is ready unless auto-play is turned off per F-83. API and MCP generation never auto-plays by itself: its audio reaches the speakers only on an explicit play request under F-89, which the owner can turn off, and which leaves the reading surface bound to whatever the user has in it.
...
| A-10 | F-46, F-50–F-52, F-89, N-17–N-19, N-31 | REST reachable on loopback only and never from another host, MCP reachable by default and still useless without a credential, and blocking of unauthenticated requests, authentication failures, revoked permissions, access to another job ID, arbitrary paths, and invalid Host and Origin values. External generation must not auto-play on the speakers: with F-89's setting off a play request is refused permanently and nothing is heard, and with it on only the job that was asked for is played, while the input text, its highlight, and the owner's own playback are left as they were. A play request arriving while the owner is playing must be refused as busy with a retry-after hint, and the owner's own play or stop must always win. |
| A-11 | F-48–F-57, F-88 | Verify REST acceptance, status, ready segments, full WAV, cancellation, error codes, and paged queries. A duplicate key must return the same job, and different content must be a conflict. Retryable responses must carry a retry-after hint, and source-text and result-metadata requests must respect their separate authorization. A request with a missing or unrecognised job kind must be refused rather than defaulted, and every job representation must state its kind, and a request with no duplicate-prevention key must be refused. A bounded wait must return the terminal state when the job finishes inside it and the job ID when it does not, must never exceed the bound, and must leave the job unchanged either way. An estimate must validate, must not create a job or load a model, and must answer within N-22's figure. |
| A-12 | F-58–F-62, N-24 | On a real MCP client, tool discovery through generation, querying, result retrieval, and cancellation all succeed, with results delivered as MCP resources over stdio. With the app not running, with REST turned off, or with an expired credential, the MCP server must fail with a distinct reason and must not launch the app. Unsupported versions and invalid input are clearly refused, and MCP must expose no capability REST does not. Capability discovery must answer without a prior handshake, and result references must carry a freshness hint within the remaining result lifetime and be marked private. Repeating a create call with the same key must yield one job and not two, every voice must come back with a description, and the bounded wait and estimate tools must behave as A-11 requires of their REST equivalents. One speak call must generate and play in a single REST operation, must report the voice actually used when the settings were omitted, and must be refused exactly as A-10 requires when playback is off; play_speech must do the same for a job created earlier. |
...
| Registering an external integration | Owner confirms REST and MCP are on, which they are unless the owner turned them off -> issue a credential for that client with least privilege -> hand the client its connection details and credential -> authenticated requests -> retrieve status and results. If the client should be able to speak aloud, confirm F-89's playback setting is on. |
...
- The application process is the core. It owns the job engine, the single generation slot, the resource budget, and the database, and it is the only component that talks to a model. The REST service is a surface over that core rather than the core itself, and it is on by default because it is the product's own service boundary, not an optional add-on.
  This corrects an earlier reading of this note, which put the engine inside the service. That could not stand: F-46 lets the owner turn REST off and F-79 requires generation, playback, and the library to keep working when its port cannot be bound, so an engine owned by the service would disappear in both cases. The surface reading is the only one consistent with those two, and building it made that unavoidable rather than arguable.
- The MCP server is a thin stdio process that is a REST client, per F-58. FastMCP is what shipped, against protocol revision 2026-07-28. It should hold no job state, no model, and no policy of its own: each tool is one REST call, and its permissions are the permissions of its REST credential.
...
| F-60, F-88, N-22 | A generation request may ask to wait a bounded time for completion instead of always returning an identifier to poll. | F-60 originally refused any blocking call because model preparation can take arbitrarily long. A.5 removed the premise for short work: warm load is 0.8 seconds and synthesis runs at about a fifth of real time, so a one-sentence request finishes well inside a second and forcing a second round trip to learn that is pure overhead for an automated caller. The bound is capped, never covers model preparation, and changes only when the response is sent, so the asynchronous lifecycle the rest of the document depends on is untouched. N-22 excludes the opt-in wait from its latency figure, because there the delay is the caller's own instruction rather than the service being slow. |
| F-49, F-54 | The duplicate-prevention key is required on every generation request rather than optional. | With one generation slot, a client that retries after a dropped or slow response would otherwise queue a second rendering of the same text behind the first, and automated clients retry as a matter of course. Making the key optional put the cost of that on whoever forgot it. There are no existing clients to break, so requiring it is free now and impossible later. |
...
| F-51, F-89, Section 5.2 | An external request may be played on the host speakers, under an owner setting that is on by default. | F-51 originally reserved the speakers for the person sitting at the machine, and read as a privacy guarantee. It was also the reason an assistant asked to say something out loud could not: it generated a file and left the owner to go and press play, which is not what "speak" means. What is given up is the absolute form of that guarantee; what replaces it is narrower and checkable -- one owner setting, a permanent refusal when it is off, a busy refusal while the owner is playing, and no path by which external audio can touch the input text or its highlight. Requiring a separate credential permission was the alternative and was rejected because it would have made every credential already issued unable to speak until it was reissued, for a decision the owner had already made once in Settings. |
| F-46, Section 4.1 | MCP is on by default rather than off. | Off by default protected a setting, not an exposure: the MCP server is a short-lived stdio process that the client starts, it listens on nothing, and without a REST credential -- which only the owner can issue -- it can do nothing at all. REST, which is the thing that actually listens, has been on by default all along. The cost of the old default was that every new client failed once, silently enough that the remedy was a documentation question; that is now one switch the owner can still turn off. |
...
What is still unverified, and who has to resolve it. None of it blocked starting implementation; the first three block release.

- Voice quality, meaning N-08 and the Section 8.2 review sample. Still nobody has listened to the output in either language, and Section 8.3 makes missing speech, unnecessary repetition, corrupted audio, and clearly incorrect sentence output release-blocking defects. What has changed is that there is something to listen to: `spikes/voice_review.py` renders the whole sample, twenty Korean and twenty English sentences plus mixed language, numbers, dates, currency, abbreviations, symbols and repeated sentences, and a comparison reel of all ten voices in each language. It also checks the three defects a machine can see, and found none: no silence, no clipping, no implausible duration. That is not what N-08 asks for. This needs ears, and it needs them early, because the lineup is a single model with no fallback.
...
- Whether `QPlainTextEdit` can replace `QTextEdit` as the reading surface. It is the cheaper widget for a 50,000-character document and its pixel behaviour is identical, but `spikes/outline_emphasis.py` cannot prove its layout stability: its `cursorRect` sweep is not idempotent, and its document-level control does not reflow far enough to measure an outline against. `QTextEdit` shipped, and `tests/test_reading_surface.py` proves the emphasis moves no glyph on the widget actually in use, so this is now an optimisation to consider if the heavier widget proves slow rather than a question anything waits on.
- The bundled typeface A.2 asks for. The mechanism ships and `scripts/fetch_fonts.py` fetches Pretendard under the Open Font License, but a plain checkout has no bundled family and falls back to Malgun Gothic or Apple SD Gothic Neo. Until a release build fetches it, a layout or synchronisation measurement taken on one operating system does not transfer to the other, which is what A.2 wanted the bundle for. `echoact.ui.fonts.load()` reports this as `metrics_are_portable`, so a recorded result can say which case it was taken under.
...
**Measured during implementation.** Four numbers the build needed that the earlier spikes had not produced.

- Speech rate, for F-81's split and F-88's estimate. Korean 6.0 code points per second of audio, English 13.3, measured at tempo 1.00x on voice F1 over twelve sentences; the spread is 3.7 to 6.4 and 7.2 to 14.0, with short sentences at the low end because onset and trailing silence are a larger fraction of them. Tempo divides duration almost exactly: 0.70x measured 1.43 times the 1.00x length and 1.50x measured 0.67, so `seconds / tempo` is the model. F-88's estimate over a mixed Korean and English passage came within 1.3 times the real duration, on the conservative side.
- Real-time factor 0.24 median and 0.34 worst case at the Section 8.2 baseline, against A.5's earlier 0.18 to 0.21. The estimate quotes the worst case, so a pre-flight answer is not optimistic about a machine busier than the one measured.
- Worker memory in the running application: 527 MiB, reported by the Windows Job Object rather than by the worker, which is the separate measurement N-21 asks for. Close enough to the 0.57 GB the feasibility spike measured to confirm the container is counting the right process.
- The engine re-chunks text on its own and its own inter-chunk silence is inert once we chunk first, both of which A.5 already recorded. Passing a large `max_chunk_length` and zero silence makes one segment exactly one engine call, which is what makes a segment's frame count its exact duration.

One measurement failed, and the failure is the result. `spikes/voice_review.py` estimates each voice's pitch to see whether F-53's descriptions could be grounded in something measured rather than invented. They cannot: the estimator disagrees with itself across languages for four of the ten voices by ratios too close to an octave to be anything else, and the ordering changed between two runs of the same voices — which is F-41's non-determinism, the same effect A.5 measured as a 0.47 maximum sample delta, turning up in a measurement of it. The descriptions stay as A.4 leaves them until a person has listened.
