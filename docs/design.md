...
| POST /api/v1/jobs | Create from text JSON or text file upload, with an explicit job kind and duplicate-prevention key; may request a bounded wait | New acceptance 202 + job ID; completed inside a requested wait 200 + terminal state; duplicate re-request 200 + existing ID |
